...
concat(
 ...
 ) AS CUSTOMER_FULLNAME, CUSTOMER_EMAIL,
...
select P.PRODUCT_ID, P.PRODUCT_DESC,P.PRODUCT_QUANTITY_AVAIL,P.PRODUCT_PRICE,
 (P.PRODUCT_PRICE*P.PRODUCT_QUANTITY_AVAIL) AS INVENTORY_VALUES,
 CASE
...
## Answer 4.
   ...
   select OC.CUSTOMER_ID, CONCAT(OC.CUSTOMER_FNAME, ' ', OC.CUSTOMER_LNAME) AS CUSTOMER_FNAME,OC.CUSTOMER_EMAIL, OC.CUSTOMER_PHONE, A.COUNTRY
   ...
   HAVING COUNT(DISTINCT(OH.ORDER_ID)= (
    select COUNT(DISTINCT OH2.ORDER_ID)
    From ORDER_HEADER OH2
    WHERE OH2.CUSTOMER_ID= OH.CUSTOMER_ID
  )
...
## Answer 5. 
     ...
     select S.SHIPPER_NAME, A.CITY, COUNT(DISTINCT OH.CUSTOMER_ID) AS NO_OF_CUSTOMERS, COUNT(DISTINCT OH.ORDER_ID) AS NO_OF_CONSIGNMENT
...
## Answer 6.
    ...
    SUM(OI.PRODUCT_QUANTITY) AS SOLD_QUANTITY, 
    CASE 
        WHEN PC.PRODUCT_CLASS_DESC IN ('Electronics', 'Computer') AND SUM(OI.PRODUCT_QUANTITY) = 0 THEN 'No Sales in past, give discount to reduce inventory'
        WHEN PC.PRODUCT_CLASS_DESC  IN ('Mobiles', 'Watches') AND SUM(OI.PRODUCT_QUANTITY) = 0 THEN 'No Sales in past, give discount to reduce inventory'
        WHEN SUM(OI.PRODUCT_QUANTITY) = 0 THEN 'No Sales in past, give discount to reduce inventory'
        WHEN PC.PRODUCT_CLASS_DESC  IN ('Electronics', 'Computer') AND P.PRODUCT_QUANTITY_AVAIL < (0.1 * SUM(OI.PRODUCT_QUANTITY)) THEN 'Low inventory, need to add inventory'
        WHEN PC.PRODUCT_CLASS_DESC  IN ('Mobiles', 'Watches') AND P.PRODUCT_QUANTITY_AVAIL < (0.2 * SUM(OI.PRODUCT_QUANTITY)) THEN 'Low inventory, need to add inventory'
        WHEN P.PRODUCT_QUANTITY_AVAIL < (0.3 * SUM(OI.PRODUCT_QUANTITY)) THEN 'Low inventory, need to add inventory'
        WHEN PC.PRODUCT_CLASS_DESC  IN ('Electronics', 'Computer') AND P.PRODUCT_QUANTITY_AVAIL < (0.5 * SUM(OI.PRODUCT_QUANTITY)) THEN 'Medium inventory, need to add some inventory'
        WHEN PC.PRODUCT_CLASS_DESC IN ('Mobiles', 'Watches') AND P.PRODUCT_QUANTITY_AVAIL < (0.6 * SUM(OI.PRODUCT_QUANTITY)) THEN 'Medium inventory, need to add some inventory'
...
## Answer 8.
    ...
    SELECT 
    OC.CUSTOMER_ID,
    CONCAT(OC.CUSTOMER_FNAME, ' ', OC.CUSTOMER_LNAME) AS CUSTOMER_FULNAME,
    SUM(OI.PRODUCT_QUANTITY) AS TOT_QUANTITY,
    SUM(OI.PRODUCT_QUANTITY * P.PRODUCT_PRICE) AS TOT_VALUE
FROM 
    ONLINE_CUSTOMER OC
...
SELECT OH.ORDER_ID, OH.CUSTOMER_ID, CONCAT(OC.CUSTOMER_FNAME, ' ', OC.CUSTOMER_LNAME) AS CUSTOMER_FNAME, SUM(OI.PRODUCT_QUANTITY) AS TOT_QUANTITY
FROM ORDER_HEADER OH
JOIN ONLINE_CUSTOMER OC ON OH.CUSTOMER_ID = OC.CUSTOMER_ID
JOIN ADDRESS A ON OC.ADDRESS_ID = A.ADDRESS_ID
JOIN order_items OI ON OH.ORDER_ID = OI.ORDER_ID
WHERE OH.ORDER_ID % 2 = 0 AND A.PINCODE NOT LIKE '5%'
GROUP BY OH.ORDER_ID, OH.CUSTOMER_ID, CUSTOMER_FNAME
